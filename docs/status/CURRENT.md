...
**Phase**: Phase C — Single-machine MVP. Physical output + surface-driven warp + project persistence are all in. Timeline UX overhaul, undo/redo for property + ripple commands, named sections, HAP decode pipeline + transcoder, playback-perf overhaul, Settings UI, **engine-global FrameCache** (replacing per-clip FrameRingBuffer), and **async D3D12 COPY queue** for texture uploads have all landed. **Phase C.12 is now an OCIO-native color pipeline** (was hand-rolled ACES; pivoted 2026-04-27 to match Disguise's direction and unlock LogC / .ocio configs / 3D LUTs out of the gate). Subtasks #1-4 are done (OcioManager skeleton + runtime DXC + FP16 compose targets + GPU processor cache); the **CI-red window opened at #3** (8/17 integration goldens fail with hash mismatches; subtask #6 will rebake). #5 is next — the heavy rendering integration that splices OCIO functions into composite_ps + mapping_surface_ps via runtime DXC. After C.12, the Director/Renderer split is the only remaining Phase D entry condition.
...
## Verified Working (as of Phase C.12 #4)

**Build**: `cmake --build build --config Release` is clean, no errors.
**Tests (unit)**: 104/104 pass — 13 FrameCache, 16 HapFormat, 8 Settings, 4 TranscodeManager, 5 OcioManager, 6 RuntimeShaderCompiler, 8 OcioGpuProcessor, plus the keyframe-roundtrip + ripple-time + decoder-interface suites.
**Tests (integration)**: 9/17 currently green — `smoke`, `png_sequence_seek`, `mixed_fps`, `cache_hit_after_seek`, `cache_budget_stress`, `screen_persistence_{save,load}`, `sections_persistence_{save,load}`. **8 fail with hash mismatches** (`multi_screen`, `seek_past_clip_end`, `ping_pong`, `blend`, `blend_difference`, `hap_roundtrip`, `hap_basic_roundtrip`, `hap_q_roundtrip`) because the C.12 #3 FP16 compose targets + sRGB-stub capture pass produce different bytes than the pre-C.12 UNORM8 path. **No crashes** — all failures are diff-hash. **Subtask 6 closes the loop** by rebaking the 22 goldens once OCIO input + display transforms also land in subtask 5.
...
- 🔄 #5 — Wire OCIO into `composite_ps` + `mapping_surface_ps` via runtime DXC *(next; the heavy rendering integration — per-input-CS PSO matrix for composite, per-(display,view) for mapping_surface + capture, splice strategy must avoid colliding with the existing t0/b0 register usage)*
