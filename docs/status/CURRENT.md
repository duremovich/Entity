...
## Verified Working (as of Phase C.12 #10 — CI green)

**Build**: `cmake --build build --config Release` is clean, no errors.
**Tests**: 135/135 pass via `ctest -C Release` — 117 unit (13 FrameCache, 16 HapFormat, 13 Settings, 4 TranscodeManager, 5 OcioManager, 6 RuntimeShaderCompiler, 8 OcioGpuProcessor, 2 DecoderColorSpace, 6 ProjectSerializer + the keyframe-roundtrip / ripple-time / decoder-interface suites) + 18 integration (all 22 goldens rebaked end-to-end through the OCIO ACES SDR pipeline + new `hap_hdr_smoke`).
...
- 🔄 #11 — `integration_aces_smoke` + OCIO ODT unit test *(next)*
