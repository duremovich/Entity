...
**Phase**: Phase C — Single-machine MVP. Physical output + surface-driven warp + project persistence are all in. Timeline UX overhaul, undo/redo for property + ripple commands, named sections, HAP decode pipeline + transcoder, playback-perf overhaul, Settings UI, **engine-global FrameCache** (replacing per-clip FrameRingBuffer), and **async D3D12 COPY queue** for texture uploads have all landed. **Phase C.12 is now an OCIO-native color pipeline** (pivoted 2026-04-27 from hand-rolled ACES to match Disguise's direction). Subtasks #1-#7 are done — OcioManager + runtime DXC + FP16 compose + GPU processor cache + rendering integration + per-decoder tagging + golden rebake + **Settings/Preferences "Color" section**. **CI green at 128/128** (123 + 5 new Settings tests). The remaining #8-#11 are all S/M and bounded (per-output display/view UI + MediaBin override + HAP HDR smoke + ACES smoke goldens). After C.12, the Director/Renderer split is the only remaining Phase D entry condition.
...
- ✅ #7 — Settings + Preferences "Color" section + process-wide `activeSettings()` snapshot for worker-thread decoder defaults (128/128)
- 🔄 #8 — Per-output display+view UI + project persistence (PROJECT_VERSION 5→6) *(next)*
