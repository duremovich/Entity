...
## Verified Working (as of Phase C.12 #8 — CI green)

**Build**: `cmake --build build --config Release` is clean, no errors.
**Tests**: 131/131 pass via `ctest -C Release` — 114 unit (13 FrameCache, 16 HapFormat, 13 Settings, 4 TranscodeManager, 5 OcioManager, 6 RuntimeShaderCompiler, 8 OcioGpuProcessor, 2 DecoderColorSpace, 3 ProjectSerializer + the keyframe-roundtrip / ripple-time / decoder-interface suites) + 17 integration (all 22 goldens rebaked end-to-end through the OCIO ACES SDR pipeline).
...
- 🔄 #9 — MediaBin per-clip color-space override *(next)*
