...
## Verified Working (as of Phase C.12 #6 — CI green)

**Build**: `cmake --build build --config Release` is clean, no errors.
**Tests**: 123/123 pass via `ctest -C Release` — 106 unit (13 FrameCache, 16 HapFormat, 8 Settings, 4 TranscodeManager, 5 OcioManager, 6 RuntimeShaderCompiler, 8 OcioGpuProcessor, 2 DecoderColorSpace + the keyframe-roundtrip / ripple-time / decoder-interface suites) + 17 integration (all 22 goldens rebaked end-to-end through the OCIO ACES SDR pipeline).
...
- 🔄 #7 — Settings + Preferences "Color" section *(next; small UI add)*
