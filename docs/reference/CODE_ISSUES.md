...
| NEW-07 | AnimationSystem.cpp / Engine.cpp:1303 | High | **AnimationSystem editor-stall freeze.** `AnimationSystem::update` only ticks from `Engine::update` on the editor thread. During any editor stall (native modal dialog, OS resize/move loop, slow project load), animated clip properties (opacity, transform, rotation, scale) freeze on the projector output even though the show thread keeps Present-ing. Same root cause as the DecodeSystem freeze fixed in `8492438`. Naive show-thread fallback would violate ADR-0014's "editor is sole registry writer" rule because AnimationSystem writes `Transform` and `MediaLayer` components. Resolution requires architectural choice: (a) bake evaluated property values into `SceneSnapshot::clipCatalog` so the show thread reads them from the snapshot, or (b) split AnimationSystem into evaluation (read-only, show-safe) + write-back (editor-only). Tracked on roadmap. **Tracy note (2026-05-10):** Once resolved, the AnimationSystem `update()` should receive `ZoneScopedN("AnimationSystem")` and the evaluated property values (opacity, transform) should appear in `SceneSnapshot::clipCatalog` so the Tracy frame timeline shows the snapshot-bake cost per editor frame. |
| NEW-08 | SectionScheduler.cpp / Engine.cpp:1278 | Medium | **SectionScheduler editor-stall freeze.** `SectionScheduler::tick` only fires from `Engine::update`. During editor stalls, section-break detection and continuation-phase advancement stall; cues fire late after the stall ends. Same registry-write constraint as NEW-07 (mutates Timeline section state and `ClipPlaybackPhase` components). Same resolution options. Tracked on roadmap. **Tracy note (2026-05-10):** `SectionScheduler::tick` should receive `ZoneScopedN("SectionScheduler")` once it is safe to call from the show thread (or when the show-thread fallback is added). |
| NEW-09 | tests/integration/ | Low | **No regression test for editor-stall fallback.** The Timeline (`cf103bd`) and DecodeSystem (`8492438`) show-thread fallbacks have no automated coverage. A future refactor of `Engine::showThreadMain` or `m_lastEditorTickNs` could silently break the feature. Need an integration test that simulates editor heartbeat staleness and asserts both systems advance from the show thread. Tracked on roadmap. **Tracy note (2026-05-10):** A baseline .tracy capture taken after issue #43 Phase 8 can serve as a reference: if the show-thread fallback breaks, the `FrameMarkNamed("Show")` intervals will stop advancing during a simulated stall. |
