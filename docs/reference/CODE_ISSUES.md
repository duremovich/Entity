...
| NEW-08 | SectionScheduler.cpp / Engine.cpp:1278 | Medium | **SectionScheduler editor-stall freeze.** `SectionScheduler::tick` only fires from `Engine::update`. During editor stalls, section-break detection and continuation-phase advancement stall; cues fire late after the stall ends. Same registry-write constraint as NEW-07 (mutates Timeline section state and `ClipPlaybackPhase` components). **Design spec**: see [`docs/design/section-scheduler-snapshot-bake.md`](../design/section-scheduler-snapshot-bake.md) — recommends a detector-on-show + applier-on-editor split with an R2D `SectionBreakDetected` message, plus a wall-clock anchor for continuation-phase advancement (no `dt` accumulator across threads). More invasive than NEW-07 because SectionScheduler is a state machine, not stateless. Implementation tracked on roadmap. **Tracy note (2026-05-10):** `SectionScheduler::tick` should receive `ZoneScopedN("SectionScheduler")` once it is safe to call from the show thread (or when the show-thread fallback is added). |
