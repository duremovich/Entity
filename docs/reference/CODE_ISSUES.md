...
| NEW-06 | D3D12Renderer device-removed handling | High | **Partially fixed — detection complete, recovery pending.** Phase A2 (#42, commit d92fd13): `handleDeviceLost` calls `GetDeviceRemovedReason()`, stores HRESULT in `m_deviceLostReason`, Engine posts `bus::DeviceLost` on R2D once and exits. Phase B (#42, 2026-05-08, ADR-0014): `handleDeviceLost` now always runs on the show thread (show thread owns all D3D12 Present calls); `m_deviceLostPosted` plain-bool is safe on the show thread (single writer). The three INFINITE waits in `D3D12Renderer::waitForGpu()` were bounded in commit `2e003d3` (2026-06-03) — the last app-level unbounded GPU wait. **This did NOT fix the integration-suite shutdown hang**: investigation (2026-06-03) showed that hang is dominated by driver-level COM release (`D3D12Device::shutdown` → command-queue/device `Reset`) on a GPU wedged by sustained back-to-back test stress (4× `DXGI_ERROR_DEVICE_REMOVED` in one `-j1` run), which application-level timeouts cannot bound. **Now split into two tracked cards**: #69 (test-isolation — stop wedging the GPU in CI) and #70 (device-loss recovery — release/recreate device, re-provision GPU resources, resync threads; survive a wedge). |
...
| NEW-08 | SectionScheduler.cpp / Engine.cpp | ~~Medium~~ **Fixed (2026-05-20); Phase G refinement (2026-05-21)** | **SectionScheduler editor-stall freeze.** `SectionScheduler::tick` used to be the sole break-crossing detector and only fired from `Engine::update`, so during editor stalls breaks fired late and continuation phase froze. **Resolution** (per [`docs/design/section-scheduler-snapshot-bake.md`](../design/section-scheduler-snapshot-bake.md), with two divergences noted there): break-crossing *detection* moved to the show thread (`Engine::showThreadMain`, the `SectionDetect` zone) — it runs every show frame regardless of editor health, snaps + pauses the playhead, raises `Timeline::sectionAtBreak()`, and posts an R2D `bus::SectionBreakDetected`. The editor *applies* the crossing via the new `SectionScheduler::handleBreakAt` from `Engine::drainRendererToDirector` (seeds `ClipPlaybackPhase`, raises the scheduler latch). Crossing detection was removed from `tick()` entirely (single detector, no dual-detector race). Continuation phase is re-derived show-side from the wall-clock anchor (`ClipPlaybackPhase::continuationStartTimeNs/SeedFrames`, baked into `bus::ClipCatalogEntry`) in `mapToMediaFrameFromCatalog`, so Loop/PingPong clips keep cycling at a parked break during a stall. `go()` recomputes the phase from the anchor before snapshotting tail-hold frames. **Phase G (2026-05-21)**: continuation anchor now set via `PlaybackTimeAuthority::rateNow()` instead of raw `steady_clock`, so editor-side seeding and show-side re-derivation share the same clock domain (audio crystal when `AudioRateSource` is active). `mapToMediaFrameFromCatalog` receives `nowNs` from `buildRenderFrame`, which snapshots `rateNow()*1e9` once per render frame. |
| NEW-09 | tests/integration/ | ~~Low~~ **Fixed (2026-07-03, issue #74)** | **No regression test for editor-stall fallback.** Closed by issue #74 alongside the fallback's registry-free rewrite (ADR-0014 amendment): `integration_decode_stall_fallback` stalls the editor 1.5 s mid-playback and asserts decode targets kept advancing (`AssertFrameCached` at a frame only reachable via the fallback) and audio didn't seek-storm on the handoff (`AssertAudioWorkerSeekCountAtMost`); `integration_load_during_playback` is the crash-class repro (LoadProject mid-playback — pre-#74 the show-thread fallback iterated registry views while load destroyed them). **Tracy note (2026-05-10):** A baseline .tracy capture can serve as a secondary reference: if the fallback breaks, `FrameMarkNamed("Show")` intervals stop advancing during a simulated stall. |
| NEW-11 | PlaybackPresenter.cpp / DecodeSystem.cpp / AudioSystem.cpp | ~~High~~ **Fixed (2026-07-03, issue #74)** | **Worker-map races.** `PlaybackPresenter::present` called `DecodeSystem::getWorker()` on the show thread every frame against an `unordered_map` the editor thread mutated (create/retire/reap) with zero synchronization — UB on rehash/erase, independent of stalls. Same pattern inside both systems' show-thread fallback re-entry. Fixed by a `TracyLockable` worker-map mutex in both systems (`getWorker` returns a `shared_ptr` copied under the lock; joins/mixer-unregister happen outside it — leaf-lock rules documented in the headers), plus deletion of dead cross-thread fields (`DecodeWorker::playbackMode`/`totalMediaFrames` write-only; `AudioDecodeWorker::playbackMode` → atomic; `AudioSystem::m_lastExpectedSample` map → per-worker atomics). |
| NEW-10 | D3D12Renderer.cpp / Engine.cpp | ~~Medium~~ **Fixed (2026-05-23)** | **Video texture slot leak.** `freeVideoTextureSlot` existed but was only ever called by `TextSystem::onTextLayerDestroyed`. Clip-allocated `VideoTexture` slots were never freed on clip deletion, so the pool leaked until process exit. Surfaced when `stress_16screen_progressive` required bumping `MAX_VIDEO_TEXTURE_SLOTS` from 16 → 256 to avoid exhausting the pool. **Fix**: added `Engine::onVideoTextureDestroyed` (mirrors `onModelDestroyed` shape) — connected in `Engine::initialize` via `m_registry.on_destroy<VideoTexture>()`, disconnected in `Engine::shutdown` before `m_rendererService.reset()`. The handler calls `IRenderer::scheduleVideoTextureSlotFree(slot)`, which on D3D12Renderer pushes the slot into `m_deferredVideoTextureFrees` (under a mutex). The show thread drains the queue at the top of `beginShowFrame` after the show fence wait, calling `TextureUploader::freeSlot` — non-blocking on the editor thread, GPU-safe on drain. Integration test: `scripts/integration/video_texture_slot_leak.json`. |
| NEW-12 | TextureUploader.cpp / D3D12Renderer.cpp / PlaybackTimeAuthority.cpp | ~~High~~ **Fixed (2026-07-09, issue #90)** | **Video-texture slot reuse against a stale cached snapshot.** The show thread caches a `SceneSnapshot` across ticks; the editor deletes clip A (`scheduleVideoTextureSlotFree(S)`) and a replacement clip B first-fits the same slot index while the cached snapshot still maps A→S. A show tick in that window uploaded A's cached frames into B's slot (and could draw B's texture as A's). #89's retiring flags + `m_slotsFreedThisTick` defend only the *same-tick* case; this is the *cross-tick* stale-snapshot case. Also closed a genuine data race the header admitted: editor-side `prepareTexture`→`ensureTexture` recreated a slot's ComPtrs while show-thread `upload`/`recordDirectCopy` read them with no lock. **Fix**: `TextureUploader::Slot` carries an `std::atomic<uint32_t> generation` bumped once per free→realloc cycle in `freeSlot`; `upload`/`recordDirectCopy` take `m_slotMutex` around their whole body and reject (returning false, bumping `m_staleGenerationSkips`) any call whose captured `expectedGeneration` no longer matches. The generation is stamped on `VideoTexture` at allocation, threaded through `ResourcesProvisioned` → `ClipCatalogEntry` → `ClipRenderState` → `ContentLayerSnapshot`, and enforced at both the upload path (`PlaybackPresenter`) and the draw path (`resolveTextureHandle` / `TextureRef::video(slot, gen)`). Integration test: `scripts/integration/video_texture_slot_reuse_stale_snapshot.json` (+ `AssertVideoTextureStaleGenerationSkips`). |
| NEW-13 | D3D12Renderer.cpp / Engine.cpp / OutputManager.cpp | ~~High~~ **Fixed (2026-07-10, issue #91)** | **waitForGpu idle contract ignored at 13 destroy-after-drain sites + shutdown's discarded drain.** `waitForGpu()` returns false on an abandoned drain (healthy device starved past the watchdog — the loaded-WARP CI regime), but 13 pre-existing callers discarded the return and `Reset()` resources potentially still referenced by in-flight command lists (resize, shutdown, output windows, capture paths, stage/OCIO recreates). shutdown()'s promised spurious-latch drain also never ran: `waitForGpu`'s bare-flag early-out returned before draining. **Fix**: drain body extracted to `drainGpuQueues(drainOnSpuriousLatch)` (shutdown drains past a spurious latch; confirmed removal aborts); `[[nodiscard]] waitForGpu` + `gpuIdleForDestroy()` helper enforce triage at every site — defer-and-retry where a loop exists (Engine deferred-resize retry ≤120 frames; OutputManager `m_pendingWindowDestroys` retried each show tick; `destroyOutputWindow` returns bool), `return false` on capture paths. **Deliberately NOT shipped**: #91 also flagged a show-fence pacing window after floored drain signals (per-ring-slot values duplicated across slots; `GetCompletedValue()` sits above them post-drain so `beginShowFrame`'s gate can pass early). A strictly-monotonic signal migration was implemented and then REVERTED during validation: real-hardware A/B runs (`stress_8screen_progressive`, GUI) revealed a PRE-EXISTING probabilistic DEVICE_HUNG on this scenario — hang rates 3/4 at pre-#91 `a746ab0`, 2/5 at #91 Phases 1-3, 5/5 with the monotonic migration (samples too small to attribute rate differences; the migration is withheld out of conservatism, preserved on branch `p91-phase4-monotonic` with its upload-recycle stamp lift). Fence-breadcrumb forensics localize the wedge to ~2 frames after the editor thread's post-load drain pair (deferred `resize` + `ensureStageTarget`, both of which drained identically pre-#91) and the first post-resize ImGui frames — consistent with the device-hung-2026-06 ImGui reuse-gate UAF class via drain-inflated editor-fence completed values bypassing `moveToNextFrame`'s stale per-slot wait targets. The pre-existing hang + pacing-window rework are tracked as a follow-up issue (see #91 close-out); DRED masks the hang (serialization heisenbug), headless/WARP never reproduces it. |
