...
// v29 — ParamValue type tag <-> string. Strings keep .entity files
// diff-friendly (same rule as blendMode / pathKind).
static std::string paramTypeToJson(ParamValue::Type t) {
    switch (t) {
...
static ParamValue::Type jsonToParamType(const std::string& s) {
...
// v29 — the "value" payload for one ParamValue: scalar for Float/Int/
// Enum/Bool, array for the vector types.
static json paramValueToJson(const ParamValue& v) {
    switch (v.type) {
...
static ParamValue jsonToParamValue(ParamValue::Type type, const json& val) {
    ParamValue p;
    p.type = type;
