...
public:
    ...
    // Used for both Clip-backed and OA-layer entities; kind discriminator
    // selects which set of fields restore reads.
    //
    ...
    struct DeletedClipSnapshot {
        DeletedLayerKind kind{DeletedLayerKind::Clip};
        entt::entity     trackEntity{entt::null};

        // Layer placement (used by both archetypes).
        FrameNumber          startFrame{0};
        FrameNumber          duration{0};
        std::string          layerName;
        std::array<float, 4> layerColor{0.4f, 0.6f, 0.9f, 1.0f};

        // Clip-only data (FFmpeg pointers excluded -- restored entity opens
        // its own). Unused when kind == ObjectAnimation.
        std::string     filepath;
        ...
        // Clip auxiliary components -- absence flag plus by-value snapshot.
